...
import org.eclipse.store.cache.hibernate.types.MissingCacheStrategy;
...
public class MyCacheRegionFactory extends RegionFactoryTemplate
{
    public static MyCacheManager CACHE_MANAGER;

    private final CacheKeysFactory cacheKeysFactory;
    private volatile MyCacheManager cacheManager;
    ...
    public MyCacheRegionFactory()
    {
        this.cacheKeysFactory = MyCacheKeysFactory.INSTANCE;
    }
    ...
    )
    {
        ...
        {
            throw new RuntimeException("Trying to initialize another cache manager, needs inspection!");
        }
    ...
    protected MyCacheManager resolveCacheManager(
        final SessionFactoryOptions settings,
        final Map properties
    )
    {
    ...
    protected MyCacheManager useExplicitCacheManager(
        final SessionFactoryOptions settings,
        final Object setting
    )
    {
        if (setting instanceof MyCacheManager)
        {
            return (MyCacheManager)setting;
        }

        try
        {
            final Class<? extends MyCacheManager> cacheManagerClass = setting instanceof Class
                                                                      ? (Class<? extends MyCacheManager>)setting
    ...
    protected MyCacheManager createCacheManager(
        final SessionFactoryOptions settings,
        final Map properties
    )
    {
        return new MyCachingProvider().getCacheManager();
    }
    ...
    )
    {
        if (defaultRegionName.equals(regionName) && !this.cacheExists(regionName, sessionFactory))
        {
            for (final String legacyDefaultRegionName : legacyDefaultRegionNames)
            {
                ...
                {
                    log(logger -> System.out.println(defaultRegionName + " " + legacyDefaultRegionName));
    ...
    )
    {
        ...
        {
        case CREATE_WARN:
            log(logger -> System.out.println(regionName + " " + ConfigurationPropertyNames.MISSING_CACHE_STRATEGY + " "
        ...
        case FAIL:
            throw new CacheException(
                "On-the-fly creation of EclipseStore Cache objects is not supported [" + regionName + "]");
    ...
    private static SecondLevelCacheLogger acquireSecondLevelCacheLogger()
    {
        ...
        catch (final NoSuchFieldException e)
        {
            ...
            {
                // Hibernate >= 6
                return (SecondLevelCacheLogger)SecondLevelCacheLogger.class.getDeclaredField("L2CACHE_LOGGER")
                    .get(null);
            }
